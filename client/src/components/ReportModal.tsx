...
// Official Google Maps drop-pin icon
const googleRedPinIcon = L.divIcon({
  className: 'custom-google-pin',
  html: `
    <div style="filter: drop-shadow(0 3px 6px rgba(0,0,0,0.4)); cursor: pointer;">
      <svg width="30" height="40" viewBox="0 0 32 42" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M16 0C7.163 0 0 7.163 0 16c0 10.625 14.28 24.625 14.89 25.22a1.5 1.5 0 0 0 2.22 0C17.72 40.625 32 26.625 32 16c0-8.837-7.163-16-16-16z" fill="#EA4335"/>
        <circle cx="16" cy="16" r="6.5" fill="#FFFFFF"/>
        <circle cx="16" cy="16" r="3" fill="#B31412"/>
      </svg>
    </div>
  `,
  iconSize: [30, 40],
  ...
  const handleDetectLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        ...
        setErrorMessage('Unable to retrieve current location. Use search or click on Google Maps to position your pin.');
        setIsLocating(false);
      },
      ...
      <div className="bg-white rounded-3xl shadow-2xl border border-slate-200 w-full max-w-xl max-h-[94vh] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-3.5 border-b border-slate-100 flex items-center justify-between bg-slate-50/80 shrink-0">
          <div className="flex items-center space-x-2.5">
            <span className="w-3 h-3 rounded-full bg-red-500 ring-4 ring-red-100 animate-pulse"></span>
            <div>
              <h2 className="text-base font-extrabold text-slate-900">
                Report Civic Hazard
              </h2>
              <p className="text-[11px] text-slate-500 font-medium">
                Google Maps GPS positioning & 50-meter clustering
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="p-5 sm:p-6 space-y-4 overflow-y-auto flex-1">
          ...
          {/* Category Selector with Clean Spacing and Full Labels */}
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {CATEGORIES.map((cat) => (
                <button
                  ...
                  className={`flex items-center space-x-2 p-2 rounded-2xl border transition text-left min-h-[46px] ${
                    category === cat.value
          ...
          {/* Location Search & Google Maps Section */}
          <div className="space-y-2">
            ...
            {/* Google-Style Location Search Bar */}
            <div className="relative">
              <div className="flex items-center space-x-2 bg-white border border-slate-200 rounded-2xl px-3.5 py-2 shadow-sm focus-within:ring-2 focus-within:ring-sky-500 focus-within:border-sky-500 transition">
            ...
            <div className="h-48 w-full rounded-2xl overflow-hidden border border-slate-200 shadow-inner relative z-0">
              ...
              >
                {/* Official Google Maps Tiles: Crisp Standard Roadmap vs High-Res Hybrid Satellite */}
                {mapType === 'gmaps' ? (
                  <TileLayer
                ...
                {/* Google Maps Iconic Red Drop-Pin */}
                <Marker position={[latitude, longitude]} icon={googleRedPinIcon} />
              ...
              <div className="absolute top-2 left-2 bg-slate-900/80 backdrop-blur-md text-white px-2.5 py-1 rounded-lg text-[10px] font-semibold pointer-events-none flex items-center space-x-1.5 shadow-md">
                ...
                <span>Click map to place red pin</span>
              </div>
